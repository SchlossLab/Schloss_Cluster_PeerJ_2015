...
#pooled_data <- pool_summaries(NULL, "miseq")
...
ggplot(pooled_data, aes(method, mean, col=dataset, shape=dataset)) +
	...
	geom_rect(aes(xmin = -Inf, ymin = 0.9, xmax= Inf, ymax = Inf), col="lightgray",
						fill="lightgray") +
	geom_text(aes(x = 5.5, y = 0.98, label = "De novo"), size = 4, color="black", fontface="plain") +
	geom_text(aes(x = 13.00, y = 0.98, label = "Reference-based"), size = 4, color="black", fontface="plain") +
	...
	xlab(NULL) +
	scale_color_discrete(breaks=c("he", "even", "staggered"),
											labels=c("Soil", "Even", "Staggered"))+
	scale_shape_discrete(breaks=c("he", "even", "staggered"),
											 labels=c("Soil", "Even", "Staggered"),
